...
func Test_writeTo(t *testing.T) {
	...
	if err := writeTo(NewSafeCSVWriter(NewWriter(e.out)), s, false); err != nil {
		t.Fatal(err)
	}
...
func Test_writeTo_Time(t *testing.T) {
	...
	if err := writeTo(NewSafeCSVWriter(NewWriter(e.out)), s, true); err != nil {
		t.Fatal(err)
	}
...
func Test_writeTo_complex_embed(t *testing.T) {
	...
	if err := writeTo(NewSafeCSVWriter(NewWriter(e.out)), sfs, false); err != nil {
		t.Fatal(err)
	}
	lines, err := csv.NewReader(&b).ReadAll()
	if err != nil {
...
func Test_writeTo_complex_inner_struct_embed(t *testing.T) {
	...
	if err := writeTo(NewSafeCSVWriter(NewWriter(e.out)), sfs, true); err != nil {
		t.Fatal(err)
	}
	lines, err := csv.NewReader(&b).ReadAll()
	if err != nil {
...
func Test_writeToChan(t *testing.T) {
	...
	if err := MarshalChan(c, NewSafeCSVWriter(NewWriter(e.out))); err != nil {
		t.Fatal(err)
	}
	lines, err := csv.NewReader(&b).ReadAll()
	if err != nil {
...
func Test_MarshalChan_ClosedChannel(t *testing.T) {
	...
	if err := MarshalChan(c, NewSafeCSVWriter(NewWriter(e.out))); !errors.Is(err, ErrChannelIsClosed) {
		t.Fatal(err)
	}
}
...
func TestRenamedTypesMarshal(t *testing.T) {
	...
	SetCSVWriter(func(out io.Writer) *SafeCSVWriter {
		csvout := NewSafeCSVWriter(NewWriter(out))
		csvout.Comma = ';'
		return csvout
	})
...
func Benchmark_MarshalCSVWithoutHeaders(b *testing.B) {
	dst := NewSafeCSVWriter(NewWriter(ioutil.Discard))
	for n := 0; n < b.N; n++ {
